...
#DESCRIPTION:	duDirSSH.sh checks via SSH authentication, if a directory on a remote host exists, then 'returns' to the caller
#				(ie. echo to standard out) the size of the files & sub-directories of that directory.  Else, an empty line is returned.
#
#				This script is entirely based on script lsDirSSH.sh
#
# CAUTION:  The script assumes the user has sufficient read permission on the target host to list directory content.
#
# SYNTAX:	bash duDirSSH.sh "/mnt" "andre@dycomserver.ddns.net" "/home/andre/.ssh/id_rsa_dycom" "2200"			(quotes optional)
#
...
function printUsage ()
{  
  echo -e "\n DESCRIPTION:	duDirSSH.sh checks via SSH authentication, if a directory on a"
  echo -e   "		remote host exists, then 'returns' to the caller (ie. echo to standard out)"
  echo -e	"		a list of the file and sub-directory contents of that directory, incl. size of each."

  echo -e "\n SYNTAX:	bash duDirSSH.sh REMOTE_DIR REMOTE_HOST SSH_KEY TCP_PORT"
  echo -e "\n PARAMS:	1: REMOTE_DIR		#absolute path to directory on remote host	(e.g. '/mnt/storage')"
  ...
  echo -e	"\n NOTE:		During SSH authentication, duDirSSH.sh cannot assume the authenticity"
...
then
  ...
  REMOTE_DIR_CONTENT=$(ssh $REMOTE_HOST -i $SSH_KEY -p $TCP_PORT "du -sch $REMOTE_DIR" 2> /dev/null)		#suppress all error messages
  ssh_exit_status="$?"
